...
def mahadist(traindata,testdata):
    ...
    # minus the X(test) with centroidTest, we will get zero-centered xrange
    testZ = []
    for ithRow in range(testMeta[0]):
        row = []
        for jthAttribute in range(testMeta[1]):
            row.append(test[ithRow][jthAttribute]/centroidTest[jthAttribute])
        testZ.append(row)
    
    # find the covariance matrix
    
    testZ = np.asmatrix(testZ)
    
    scatter = testZ.dot(np.transpose(testZ)) 
    cov = scatter / testMeta[0]
